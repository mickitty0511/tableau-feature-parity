...
Tableau公式ヘルプには、[Web 作成と Tableau Desktop 機能の比較](https://help.tableau.com/current/online/ja-jp/server_desktop_web_edit_differences.htm)があります。これは重要な出発点ですが、公式ページ自体も基本的な作成機能の要約であり、Desktop環境とWeb環境のすべての差分を扱うものではありません。

組織内でTableauを使うときに実際に困るのは、単に「機能があるかないか」だけではありません。どの機能で、どの操作ができるのか、できないのか、メニューや操作感がどう違うのか、そしてその差分が編集・書式設定・公開・保守・引き継ぎの快適性にどう影響するのかを、比較して把握できる必要があります。

このプロジェクトは、そうした現場の小さく見えて大きい摩擦を、実操作に基づいて残すための取り組みです。検証、スクリーンショット更新、バージョン差分確認、記事保守には継続的な時間が必要です。スポンサー支援は、同じつまずきを各組織が何度も再発見しなくて済むようにする、実務向けの知識基盤の維持につながります。

## このプロジェクトが作るものと作り方

このプロジェクトでは、Tableau CloudとTableau Desktopを実際に操作して観測できた機能差分や操作差分を記事化します。単に対応可否を並べるだけでなく、どの場面で操作体験が変わるのか、業務でどう注意すべきかを閲覧者が判断できる形で整理します。

目指しているのは、困ったときに「まずこれを見る」と展開できる比較資料です。誰でも共有し、閲覧し、Tableauを使った業務の設計、レビュー、移行検討、社内サポート、チーム教育に活かせる状態を保つことを目的としています。
